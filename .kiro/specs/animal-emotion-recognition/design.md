...
본 문서는 Kaggle "Pets Facial Expression Dataset" 기반의 딥러닝 모델을 활용하여 동물 이미지에서 감정 상태를 한국어로 표현하는 모바일 애플리케이션의 기술 설계를 정의한다. 해당 데이터셋은 소규모(총 약 1,074장)이므로 데이터 증강 전략을 적극 활용한다.
...
/// 동물 미감지 오류
class AnimalNotDetectedError extends ClassificationResult {
  const AnimalNotDetectedError();
}
...
class FormattedResult {
  final String sentence;              // 한국어 감정 문장
...
class DataAugmentor:
    ...
    def augment(self, images: list, category: str) -> list:
        """원본 이미지에 증강 기법을 적용한다
...
```dart
/// 기기 메모리 상태를 모니터링하는 서비스
abstract class MemoryMonitor {
  /// 현재 가용 메모리(MB)를 반환한다
  Future<int> getAvailableMemoryMB();

  /// 메모리 부족 여부를 확인한다 (50MB 이하)
  Future<bool> isMemoryLow();
...
class AnalysisHistoryEntry {
  final int? id;
  final String imageThumbnailPath;  // 썸네일 경로
...
DATASET_CONFIG = {
    ...
    "augmentation": {
        "min_expansion_ratio": 4,  # 최소 4배 확장
        "techniques": [
            "rotation_30",         # 회전 ±30도
...
*임의의* 유효한 모델 원시 출력(raw logits)에 대해, 후처리된 결과는 항상 (a) 정확히 4개의 감정 카테고리(Angry, Happy, Sad, Other)를 포함하고, (b) 각 카테고리의 확률값 합계가 100%이며, (c) 반환된 최상위 카테고리가 실제로 가장 높은 확률을 가진 카테고리여야 한다.
...
*임의의* 감정 카테고리와 동물명 조합에 대해, 생성된 결과 문장은 (a) "[동물명]이(가) [감정 표현] 것 같아요" 구조를 따르며, (b) 사용된 감정 표현이 해당 카테고리의 정의된 표현 템플릿 목록에 포함되고, (c) 표시된 이모지가 해당 카테고리에 매핑된 이모지와 일치하며, (d) 신뢰도 점수가 정수 백분율로 표시되어야 한다.

**Validates: Requirements 3.1, 3.2, 3.3**

### Property 6: 불확실 결과 포맷팅

*임의의* 불확실 판정 결과(신뢰도 50% 미만)에 대해, 표시되는 상위 3개 항목은 각각 (a) 올바른 감정 이모지, (b) 한국어 카테고리명, (c) 정수 백분율 신뢰도를 포함하며, 내림차순으로 정렬되어야 한다.

**Validates: Requirements 3.4**

### Property 7: 이력 저장 라운드 트립

*임의의* 유효한 분석 이력 항목(썸네일 경로, 감정 카테고리, 신뢰도, 분석 일시)에 대해, 저장 후 조회하면 모든 필드가 원본과 동일하게 보존되어야 한다.
...
*임의의* 학습 이미지 세트에 대해, 데이터 증강 적용 후 결과 데이터 수는 원본 데이터 수의 최소 4배 이상이어야 한다.
...
*임의의* 불균형한 4개 카테고리 데이터셋에 대해, 데이터 증강 및 균형 조정 후 모든 카테고리의 학습 데이터 수 차이가 최대 카테고리 대비 20% 이내여야 한다.
...
*임의의* 분석 이력 목록에서 메모리 부족으로 캐시 정리가 수행될 때, 삭제되는 항목은 항상 분석 일시가 가장 오래된 항목부터 순차적으로 선택되어야 한다.
...
| 동물 미감지 | EmotionClassifier | 분류 수행 중단 | "이미지에서 동물을 찾을 수 없습니다" |
| 모델 추론 실패 | EmotionClassifier | 이미지 데이터 보존, 재시도 허용 | "감정 분석 중 오류가 발생했습니다" |
| 메모리 부족 | MemoryMonitor | 캐시 정리 후 재시도 | "메모리 부족으로 분석을 수행할 수 없습니다" |
...
flowchart TD
    ...
    CheckMemory -->|부족| ClearCache[캐시 정리]
...
- **라이브러리**: `dart_check` (Dart용 PBT 라이브러리) 또는 `glados` (앱), `hypothesis` (Python 학습 파이프라인)
...
| 5 | ResultFormatter.format | 임의 카테고리/동물명 조합 |
...
| 14 | HistoryManager 캐시 정리 | 임의 시간 순서의 이력 목록 |
...
- **감정 분류**: 동물 미감지, 추론 실패 시나리오
- **결과 표시**: 오류 상태 UI, 로딩 상태
- **UI 위젯**: 버튼 존재 여부, 결과 화면 구성
...
│   └── cache_deletion_order_test.dart       # Property 14
├── widget/
